...
| **Watch (quality leader, can't train yet)** | **LeVo 2 / SongGeneration 2** (Tencent, 4B hybrid LLM+diffusion) | Beats ACE-Step on every quality axis (OVL 3.42 vs 2.26, MEL/HAM/SSC all higher; PER 8.55%) and listening tests say it **avoids ACE-Step's "metallic vocal register"** = our exact Yannis failure mode. Rock/Metal explicitly supported. | **Non-commercial / academic-only** (fine for our personal use; vs ACE-Step Apache-ish) |
| **Optional, only if growls ever matter** | **YuE** | Only open model that does genuine death growls/screams | Apache 2.0 |

**LeVo 2 (researched 2026-06-10), why it's WATCH not adopt:** (1) **No fine-tuning/LoRA** — finetuning scripts are an unreleased TODO; only a 10s reference clip (a weak per-gen timbre nudge, NOT a trained full-artist voice — so it can't serve [[lora-goal-full-artist-sound]] today). The reference path is stem-free (full clip, no vocal isolation) so it's NOT the [[vocal-stems-unusable]] trap, worth a quick try. (2) **VRAM**: base/base-new fit the 3090 (10-16GB) but the quality-leading **v2-large (4B) wants 22GB / 28GB-with-prompt = borderline-to-over 24GB**; watch for **v2-medium** (coming). (3) Linux-first -> **WSL2** on our box (YuE-like friction). (4) RTF 0.67-0.82 on datacenter H20, slower on 3090. **TEST IDEA (own WSL2 session, not while a GPU run is live [[no-concurrent-clap-engine]]):** A/B base-model + 10s-Yannis-reference output vs ACE-Step by ear on the Beast in Black case, to validate the "less metallic vocals" claim. **REVISIT TRIGGER:** finetuning-script release (would let us train artist adapters) and/or v2-medium (24GB-friendly). Sources: github.com/tencent-ailab/SongGeneration, arXiv 2506.07520.
...
**Deep dive — repaint/extend approach (2026-05-25, user wants these features; research-first):**

_Model-level truth (from the ACE-Step-1.5 pipeline source, `acestep/inference.py` + `constants.py`)._ ACE-Step tasks = `text2music, repaint, cover (/cover-nofsq), extract (stems), lego (multi-track layers), complete (auto-complete = extend)`; Turbo modes = Simple/Custom/**Remix**/Repaint. Repaint params: `repainting_start`/`repainting_end` (sec; -1=to end), `chunk_mask_mode` (explicit 0/1 mask vs auto), `repaint_latent_crossfade_frames` (~10 ≈0.4s @25Hz, boundary blend), `repaint_wav_crossfade_sec`, `repaint_mode` (conservative/balanced/aggressive), `repaint_strength` (0=aggressive..1=conservative). Retake: `retake_seed` + `retake_variance` (0=no-op, variance-preserving noise mix for variations). Lyric/flow-edit: `flow_edit_n_min`/`n_max` (overlay window). So repaint = re-noise + regenerate a masked latent time-region, preserving the rest, with crossfade blending at the boundary.

_Three implementation routes for OUR setup (native ComfyUI on :8188 + thin FastAPI client):_
1. **Core-native masked latent (no install).** `LoadAudio→VAEEncodeAudio→SetLatentNoiseMask(time region)→KSampler(denoise, new tags/lyrics)→VAEDecodeAudio`; extend = pad latent with silence/empty frames then repaint the new region. Our box HAS the core mask nodes (`SetLatentNoiseMask`, `LatentComposite`, `LatentBlend`, `LatentCompositeMasked`). **BLOCKER/UNKNOWN:** core ComfyUI's ACE tutorial documents ONLY text2music + audio2audio — NOT repaint/extend; and authoring a **1-D time-axis mask for an audio latent** (latent is [B,C,T]) has no obvious core node (mask nodes are image H×W). So true latent-repaint via stock nodes is unverified/fragile — would need a mask hack or a tiny custom node.
2. **Custom ACE repaint/extend node pack on the box (recommended for TRUE repaint/extend).** The polished community repaint workflows use pack nodes like `ACEStepRepaintGuider` / `ACEStepAudioPostProcessor` (RyanOnTheInside) or `billwuhao/ComfyUI_ACE-Step` (+`ACEStep15SilenceLatent` for extend padding). One git-clone + ComfyUI restart (same pattern as Detail Daemon), keeps our :8188 architecture, gives coherent latent-level repaint/extend we drive from the client. **Need to verify which pack targets ACE 1.5 XL + works with our `xl_base/xl_sft` checkpoints before installing.**
3. **ACE-Step-ComfyUI server pack (heaviest).** `ace-step/ACE-Step-ComfyUI` is **server-based** (cloud API key OR a separate local ACE-Step-1.5 inference server on :8002, its own model download) with first-class repaint/cover/remix params (`is_repaint`, `repaint_start/end`, `cover_strength`, `remix_strength`; DiT guidance 3.5 / 60 steps). Full task suite but a parallel heavy system to our ComfyUI — overkill unless we want all tasks.

_Cheap adjacent wins (no install):_ **retake/variation** = audio2audio (our Restyle path) at LOW denoise + new seed ≈ ACE's retake_variance; **waveform "poor-man's repaint/extend"** = generate a fresh section / continuation via text2music and splice with our existing `mix.py` crossfade/stitch — honest splice (not seamless latent-coherent), but 100% native and reuses built code as a v1.
...
- **G. Already covered:** "Inspire Me" ≈ our **Generate + Song Constructor + lyric writer** (NOT the Assistant dock — corrected in §11a); "Music Enhancer" ≈ our **Restyle** (style-transfer/reimagine with a consistency slider — NOT Matchering; corrected in §11a); stem-split ≈ Demucs; voice cloning ≈ RVC + SoulX; lyric editing ≈ Repaint. _(§11a re-maps these from the actual ACE Studio docs.)_
...
### 11a. ACE Studio comparative deep-dive — verified from the official docs (2026-05-25)

_Read the actual ACE Studio docs/reviews for the four features the user flagged ("they may do it better"). Two §11 mappings were WRONG and are corrected here. Sources at the end._

**Add a Layer (= `lego`) — confirms our build approach.** The docs are explicit: it "generates a new **isolated** audio layer rather than replacing existing content … layered on top of the existing project" (additive, not regeneration). User inputs: **(1)** click-drag a time region on the canvas, **(2)** pick a **Layer Type** = Song Track / Vocal / Backing Vocals / **Custom (name the instrument)**, **(3)** a short **style description**, **(4)** optional lyrics (for vocal types). "The AI will analyze your project and generate a brand-new matching layer." Both Add-a-Layer and Music Enhancer are flagged **beta** and "hit-and-miss" in reviews.
  - ⇒ **Implication for `build_lego`:** ACE Studio's UI returns an isolated additive stem; we expected the same. **GPU run (2026-05-25) RESOLVED this: our vendored `ACEStep15NativeLegoGuider` instead returns a COMPLETE MIX with the new part baked into the region** — mel-cosine 0.94–0.96 + RMS ratio ≈1.0 OUTSIDE the region (backing preserved), vs cosine 0.756 + RMS ratio 1.37 INSIDE (new guitar layered on top). So **no separate `mix.py` step is needed** — `build_lego`'s output is saved as-is (more convenient than ACE Studio's stem+mix flow). Their "Layer Type" maps onto our `track_name` enum (vocals/drums/bass/guitar/keyboard/strings/synth/fx/brass/woodwinds/backing_vocals); "Custom + instrument name" → we lean on free-text style tags since the node's track_name is a fixed enum.

**Music Enhancer (CORRECTED: ≈ our Restyle, NOT Matchering).** It is **style-transfer / creative "reimagine," not mastering/quality-uplift.** You select audio on the canvas; it auto-analyzes the selection's **lyrics + style tags** (e.g. "funky drums, brass, organ"), you rewrite the style box ("symphonic orchestra, epic choir, cinematic strings"), and a **consistency slider** controls "how closely the new track follows the original." That is precisely our **Restyle** (`build_restyle`, `restyle_amount` = denoise = their inverted consistency slider). _What we could steal:_ (a) **auto-analyze the source** and pre-fill the style/tags box (we make the user type tags blind) — could use our librosa/`sections.py` + a quick LLM tag-guess; (b) rename our opaque `restyle_amount` to a **"Consistency"** slider (high = faithful) for clarity. Matchering stays our true mastering stage (no ACE Studio equivalent — a genuine Crucible advantage).

**Inspire Me (CORRECTED: ≈ our Generate + Song + lyric writer, NOT the Assistant dock).** It is **text-to-music** with two modes: **From Idea** (describe a vibe → playable sketch) and **From Lyrics** (paste lyrics + a separate Styles/Tags box → a song matching both). That is exactly our **Generate** tab (tags + lyrics) and the **Song Constructor + ✨ lyric writer**. We already match it; their only framing win is the explicit **two-mode entry** ("idea" vs "lyrics-first"). Our Assistant dock is a *separate* idea-brainstorming chat — closer to a creative co-pilot than to Inspire Me.

**Piano-roll vocal editor (their biggest genuine lead over us).** Our Vocal Builder roll is **read-only SVG**; theirs is a full sculpting surface. Verified toolset:
  - **AI pitch vs user pitch:** AI generates a pitch curve from notes+lyrics+voice (dark curve); user edits **override** it (white curve). A **Fixed Brush** locks chosen AI segments so they survive regeneration.
  - **6 pitch tools:** Pitch Brush (draw/erase, tablet-friendly), Pitch Anchor (click-to-place points), Fixed Brush (lock), Pitch Eraser, **Vibrato** (drag to add; 4 handles = Amplitude / Phase / Frequency / Attack-Release envelopes), **Modulation** (vertical drag flattens or expands the curve's dynamic range). **Voiceless/breathy pitch** via Alt-draw.
  - **6 expressive automatable params:** Formant, Energy, Tension, Falsetto, Air, Breath — "between pitch-correction and full performance direction."
  - ⇒ _Opportunity (UI-heavy, later):_ make our SVG roll **editable** — at minimum draggable note pitch/start/length + a per-note vibrato depth/rate, written back into the melody score (`backend/melody.py` already produces degrees→MIDI) before synth. We won't match their f0-curve brush soon, but **note-level + vibrato editing** is achievable and would close most of the practical gap, since SoulX already supplies the performed vibrato/expression from the score.

**Multitrack canvas vs our tabbed UI.** ACE Studio is a true DAW timeline (Audio Tracks + Singer Tracks on a horizontal canvas; layers/enhancer operate on canvas selections). We are tab/workflow-oriented (Create/Guitar/Vocals/Finish). A full canvas is a large rebuild and arguably off-mission for a power-user metal pipeline; the **achievable steal** is making the Library drawer + Mix tab feel more timeline-like (region selection driving Layer/Repaint), rather than a from-scratch canvas. Defer.

**Net adoptable list (post-deep-dive):** (1) **Add-a-Layer** = build now (lego, isolated stem → mix). (2) **Restyle UX uplift** = rename to "Consistency" + auto-analyze/pre-fill source tags (cheap, Mac-side). (3) **Editable vocal piano-roll** (note drag + vibrato) = the real quality gap, UI-heavy, schedule later. (4) Inspire Me two-mode framing = minor Generate tweak. Matchering (our master) has no ACE Studio analog — keep promoting it.

**Sources (§11a):** Add a Layer https://docs.acestudio.ai/ai-tools/add-a-layer · Music Enhancer https://docs.acestudio.ai/ai-tools/music-enhancer · Inspire Me https://docs.acestudio.ai/ai-tools/inspire-me · Editing Pitch https://docs.acestudio.ai/ai-vocal-synth/creating-ai-vocals/editing-pitch · Editing Notes https://docs.acestudio.ai/ai-vocal-synth/creating-ai-vocals/editing-notes · Tour the Interface https://docs.acestudio.ai/getting-started/tour-the-interface · review (params Formant/Energy/Tension/Falsetto/Air/Breath; beta notes) https://www.soundonsound.com/reviews/ace-studio-2 · https://bedroomproducersblog.com/2025/12/18/ace-studio/

### 11b. ACE Studio 2.0 — FULL feature sweep + adoptable list (2026-05-27)

_User asked, very thoroughly, "what else does ACE Studio do that we could do too?" This is the complete 2026 ACE Studio 2.0 feature surface mapped to Crucible. §11/§11a already covered Add-a-Layer, Music Enhancer, Inspire Me, the piano-roll editor and the canvas; below is everything NOT yet catalogued, with a have/partial/missing verdict and local-stack feasibility._

**Full ACE Studio 2.0 feature inventory** (acestudio.ai + docs.acestudio.ai, Verse25 era):
vocal synth from MIDI+lyrics (140+ Verse25 voices, 8 langs, genres incl. **Country/Rap/Afro/Theatre**); editable **piano-roll** (pitch/timing/vibrato/pronunciation + emotion params Formant/Energy/Tension/Falsetto/Air/Breath); **Choir Mode** (assemble many AI voices into choir types); **Harmony** (auto lead+harmonies+chorals); **VoiceMix** (blend voice models, no training); **Voice cloning** (train a singer from your samples); **Voice Changer** (vocal→another voice/instrument); **AI Instruments** (Chorus25 strings/brass/duduk, proprietary) + **Ensemble Mode** + articulation; **Generative Kits** (Layer Generator, Music Enhancer, Inspire Starter, **Text→Samples** loops); **Stem Splitter**; **Vocal→MIDI + lyric extraction**; **Audio→MIDI**; **PDF→MusicXML** (score OMR); **Video→Music / Video→SFX / Video Composer Agent**; **Turbo Mode + pre-rendering**; **ACE Bridge** DAW plugin (VST3/AU/AAX, ARA tempo-sync); redesigned **canvas** timeline.

**Mapping to Crucible:**
- ✅ **Already at parity (or ahead):** vocals-from-melody+lyrics → Vocal Builder+SoulX; stem split → Demucs/**RoFormer (SOTA, ahead)**; voice changer → RVC+SoulX; Music Enhancer → Restyle; Inspire Me → Generate+Song+lyric writer; Add-a-Layer → lego; Turbo → turbo model; Text→Samples ≈ short Generate clips. **Things they LACK that we have:** Matchering **mastering**, the **Helix/clean-DI guitar-amp** pipeline, **metal-first instrumental** generation, fully **local/private/free**.
- 🟡 **Partial:** voice cloning (we have RVC trained voices + SoulX zero-shot, but no "train your own singer" UI); piano-roll (ours read-only — their editable roll is the real gap, §11a); vocal→MIDI+lyrics (we have Demucs vocal + Whisper ASR; missing the audio→MIDI piece).
- ⬜ **MISSING & worth adopting (prioritized, all feasible on our stack):**
  1. **Harmony / gang vocals / choir (HIGH, very on-mission for metal).** Auto-generate harmony lines (in-key 3rds/5ths/octaves) from a lead melody and stack multiple SoulX/RVC renders (slight detune+timing humanize, pan) → harmonized leads, **gang/shout vocals**, **symphonic choirs**. Builds directly on `backend/melody.py` (theory) + Vocal Builder + `mix.py`. Combines ACE Studio's *Harmony* + *Choir Mode*.
  2. **Harmonized guitar leads (HIGH, metal signature — Maiden/thirds).** Extend the Guitar pipeline: generate a parallel 3rd/6th line from the lead (melody theory already exists) and render+pan both. On-mission, builds on `guitar.py`.
  3. **Vocal doubling (MED, cheap, standard metal production).** Render a vocal take twice (different seed / micro-timing+pitch jitter), pan L/R for a thick double-track. Trivial on SoulX/RVC + mix.
  4. **Audio→MIDI via Basic Pitch (MED; Spotify, MIT, Mac-CPU).** Unlocks: vocal→MIDI+lyrics edit (Demucs vocal → Basic Pitch melody → Whisper lyrics → re-sing via SoulX = ACE Studio's "edit vocal from a mixed track"), hum→guitar, and feeding the (future) editable piano-roll. Was already flagged "planned §4".
  5. **Editable piano-roll (MED-HIGH UI; the real vocal gap, §11a).** Draggable note pitch/start/length + per-note vibrato depth/rate written back to the score before SoulX synth.
- ❌ **Not adopting / off-mission:** proprietary singer (Verse25) + instrument (Chorus25) weights — closed; we use ACE-Step/SoulX/RVC + ACE-Step string/orchestral generation/lego instead. **Video→Music/SFX/Composer Agent** — off-mission for a metal studio. **ACE Bridge DAW plugin** — big separate build; we already export MP3/WAV/MIDI/stems. **PDF→MusicXML** (niche OMR), **VoiceMix** (timbre-blend; low feasibility on RVC/SoulX) — low priority.

**Recommendation:** the standout new adopt is **(1) harmony/choir + (2) harmonized guitar leads** — both lean on our existing melody-theory + synth/guitar code, are unmistakably *metal* (gang vocals, choirs, twin-lead harmonies), and have no real Crucible equivalent yet. **(3) doubling** is a cheap quick win. **(4) Basic Pitch audio→MIDI** is the enabling primitive for vocal-to-MIDI editing and the editable roll **(5)**. Keep promoting our genuine edges they lack (mastering, guitar-amp DI, local/free).

**Sources (§11b):** ACE Studio 2.0 overview https://acestudio.ai/ · What's New https://docs.acestudio.ai/getting-started/whats-new · KVR 2.0 (140 voices/instruments/kits) https://www.kvraudio.com/news/timedomain-releases-ace-studio-2-0---adds-140-ai-voices-new-instruments-and-generative-kits-65620 · Sound on Sound 2.0 https://www.soundonsound.com/news/ace-studio-20-timedomain · review/feature list https://singify.fineshare.com/blog/ai-music-apps/ace-studio · voice cloning https://docs.acestudio.ai/voice-cloning/cloning-a-voice-for-vocal-synth · vocal-from-MIDI/audio https://acestudio.ai/blog/generate-vocal-from-midi/ · Basic Pitch (audio→MIDI) https://github.com/spotify/basic-pitch

## 12. Better models per step — separation upgrade + a STANDING PRINCIPLE (2026-05-25)

**Standing principle (user directive, 2026-05-25):** for EVERY pipeline step, proactively check whether a better model/tool exists and propose it — Mac-native Python is fine, but if the GPU (3090/ComfyUI) makes it better, always offer that too. The user happily installs new models/nodes if they improve quality. Keep designs engine-agnostic so models are *selectable options*, not replacements. (Mirrored in project memory `feedback_always-check-better-model`.) Apply this to separation, generation, vocals, mastering, tone, audio→MIDI, etc.

### 12a. Music source separation — current best (for the Add-a-Layer "isolate stem" step and Stems/Backing)

_Our current separators: Demucs `htdemucs` / `htdemucs_6s` (Mac MPS). Research for a quality upgrade, esp. for GUITAR isolation._

**State of the art (2026): RoFormer-family models beat Demucs.** On MUSDB18HQ, **BS-RoFormer** ≈ **9.76 dB** avg SDR vs HTDemucs **7.68 dB** (+2.08 dB, biggest wins on bass). **Mel-Band RoFormer** beats BS-RoFormer on vocals/drums/other (mel-scale front-end). Demucs remains a fine fast/low-latency default, but for offline best-quality stems RoFormer is the upgrade.

**Directly relevant to us — a native 6-stem guitar model:** **"BS-Roformer SW"** separates **vocals / bass / drums / guitar / piano / other** in one pass (mvsep.com/algorithms/77) — exactly our `track_name` targets, and purpose-trained (better guitar isolation than Demucs `htdemucs_6s`'s guitar stem). **ZFTurbo/Music-Source-Separation-Training** is the canonical repo: pretrained BS/Mel RoFormer checkpoints incl. the 6-stem set, plus a **53-stem** model that even splits **acoustic-guitar vs electric-guitar**. Configs keyed `bs_roformer` / `mel_band_roformer`.

**Ways to run it (GPU):**
- **`kijai/ComfyUI-MelBandRoFormer`** — Mel-Band RoFormer as ComfyUI nodes (`MelBandRoFormerModelLoader` + `…Sampler`), GPU. Stays in our ComfyUI architecture; best for vocals/drums/other. Mel-Band specific.
- **`set-soft/AudioSeparation`** — ComfyUI node, **MDX-Net + Demucs** (46 UVR5 models incl. the HT-Demucs **6-source** guitar/piano), **auto-downloads** to `ComfyUI/models/audio/`, GPU via `target_device`. Easy install, broad model menu, but MDX/Demucs (not RoFormer).
- **ZFTurbo MSST repo as a small service on the box** (like our RVC/SoulX servers) — runs ANY MSST config incl. the 6-stem **BS-Roformer SW** guitar model and the 53-stem model. Most flexible for the *guitar-specific* RoFormer checkpoint; a parallel service to ComfyUI (own venv + model download).

**Recommendation:** add a **separation backend option** to the isolate step (and later Stems/Backing), engine-agnostic: keep `demucs` (Mac, fast, no GPU) as the default, add a **`roformer` (GPU)** option = the **6-stem BS-Roformer SW** (vocals/bass/drums/guitar/piano/other) for best guitar isolation. Deploy via whichever is least friction while giving the guitar model — likely the ZFTurbo checkpoint (either through a general ComfyUI MSST node if one cleanly loads it, or a thin box-side service). **Confirm install/GPU before doing it** (no-surprise rule). Verify by the same gated-stem RMS/mel-cosine method we used for Demucs vs ACE-extract (§ Add-a-Layer verification).

### 12b. BS-Roformer SW deployment — Mac MPS CRASHED, runs on the box (2026-05-25)

**Model:** open weights `jarredou/BS-ROFO-SW-Fixed` (config `BS-Rofo-SW-Fixed.yaml` + `.ckpt`, ~672 MB), 6 stems (vocals/bass/drums/guitar/piano/other). Runner: `pip install bs-roformer-infer` (Python ≥3.10; CLI `bs-roformer-download` / `bs-roformer-infer --config_path --model_path --input_folder --store_dir --device`).

**⚠️ Do NOT run it on the Mac.** First Mac attempt (MPS, Python 3.12 venv on the SSD) **hard-crashed/rebooted the Mac** — BS-Roformer is a big transformer and MPS unified-memory pressure caused a kernel panic. Demucs is fine on MPS; RoFormer is not. ⇒ **RoFormer runs on the 3090 only.**

**Deployment = box-side service** (mirrors RVC/SoulX), NOT a ComfyUI node (no community node cleanly loads the 6-stem SW config; existing ones are Mel-Band/2-stem). Files (this repo): `ROFORMER-API_AUTO_INSTALL.bat` (box: venv + CUDA torch + bs-roformer-infer + SW model + launcher), `backend/roformer_server.py` (box FastAPI: `/health`, `/separate` → base64 stems; runs the CLI per request so VRAM frees after each), `backend/roformer_py.py` (Mac client). Mac calls `C.free()` first to release the 3090 from the ACE-Step models (box was 0.3 GB free with ACE loaded). Engine-agnostic `_separate()` in `app.py` dispatches `demucs` (Mac) vs `roformer` (box); wired into `/api/layer/isolate` (method `roformer`), `/api/stems/separate` (`engine`), `/api/backing/strip-guitar` (`engine`). `/api/config` reports `roformer` availability (set `roformer_host` in `app_config.json`); UI shows the engine option only when present.

**Disk note:** the Mac system drive is small (~6 GB free at the time). All project model/cache downloads are pinned to the SSD via `run.sh` env (`TORCH_HOME`/`HF_HOME`/`PIP_CACHE_DIR`/`UV_CACHE_DIR` → `.caches/`); the unrelated 3.9 GB `~/.cache/huggingface` was relocated to `/Volumes/SSD1/caches/huggingface` (symlinked). The box has its own disk — the installer downloads CUDA torch + the model there.
...
**Sourcing LoRAs without Civitai (2026-05-25):** Civitai is geoblocked from Claude's web tools, but it is NOT the main hub for ACE-Step LoRAs — **HuggingFace is**, and Claude CAN reach/search/fetch HF directly (the raspy+distorted pack, acoustic-guitar LoRA, official LoRAs, GGUF quants all live there). **ModelScope** (Alibaba) is the secondary official mirror and the engine can pull from it natively (`--download-source modelscope`) — a Civitai-independent fallback. Tensor.Art/LiblibAI/Shakker are mostly image LoRAs (thin on ACE-Step music). ⇒ Default to HF for discovery+download; ModelScope as mirror; Civitai only for its exclusives (e.g. Epic Music), handled by the user's VPN-download→transfer ([[civitai-vpn]]) — or a VPN route to let Claude browse Civitai's catalogue directly if desired later.

## 16. AI / reference-free "gold standard" mastering — options research (2026-05-27)

_User wants a **reference-free** master ("a gold-standard master applied to any track without a specific reference in mind"), on top of our current Matchering (which needs a reference). Researched the 2026 landscape — local/open vs commercial-local vs cloud — with the reference-free + private/local lens._

**How "reference-free" AI mastering actually works (concept):** trained on large catalogues of released masters, these systems learn a target tonal balance + loudness and push any input toward it, then run a **loudness re-measure loop** to a target (re-measure integrated LUFS, iterate) with safety clamps. Streaming targets cluster at **−14 LUFS** (Apple ~−16); modern "loud" masters sit **−8 to −10 LUFS**. So the building blocks are: matching EQ → glue/multiband compression → true-peak limiter → loudness normalize to a chosen LUFS.

**Options, ranked for Crucible (local-first, private, free):**
- **① Matchering + a curated "gold-standard" reference set (LOWEST effort — reuses what we have).** Matchering is reference-based, but we can bundle/curate a few hand-picked pro masters (per subgenre: power/thrash/doom/symphonic…) and pick one automatically → reference-free *from the user's POV*. Zero new deps (Matchering already installed, MIT-ish/GPL), Mac-local, deterministic, real-reference quality. The reference is just hidden. **Recommended quick win.** (Could also let the user flag any library track as a "gold standard ref".)
- **② Reference-free DSP chain via `pedalboard` + `pyloudnorm` (Crucible-native, no external master needed).** We already use Spotify **pedalboard** for Helix hosting; it has Compressor/Limiter/Gain/HighShelf/LowShelf/PeakFilter + VST3 hosting. Build: analyze the track's spectrum → gentle matching EQ toward a **target tonal curve** (genre presets) → multiband/glue compression → **true-peak limiter to a target LUFS** (selectable −14 streaming / −9 loud) measured with `pyloudnorm`, with an iterate-to-target loop. Fully reference-free, tunable, local, light deps. **Best "no reference at all" engine; medium build.**
- **③ phaselimiter (`ai-mastering/phaselimiter`, MIT) — open *learned* reference-free master.** The C++ engine behind bakuage / ai-mastering.com's free auto-master; fully automatic, no reference, CC0 training data. **MIT** = usable. Cons: C++ build (libtbb/lapack/armadillo/libsndfile/ffmpeg + CMake), **no macOS prebuilt** (Linux/Windows prebuilts exist — could run on the **Windows box** as a service, or Docker), maintenance "inactive". **Highest-fidelity open reference-free option but heaviest to integrate.**
- **④ iZotope Ozone Master Assistant (commercial, LOCAL plugin) — host via pedalboard like Helix.** The pro "gold standard" reference-free local AI master (analyzes the track, builds an EQ/dynamics/loudness chain, targets a loudness, override-able; Clarity module new 2026). If the user owns Ozone (VST3), pedalboard can host it exactly like our Helix Native integration. **Best quality, paid, but fits our plugin-hosting pattern.**
- **⑤ DeepAFx-ST (adobe-research) — neural production-style transfer.** PyTorch, pretrained music checkpoints, CPU-feasible (GPU ~5×). **Reference-REQUIRED** at inference (so pair with a fixed gold-standard ref, like ①) and the **Adobe Research License is restrictive** (research/non-commercial) → poor fit for a tool we run freely. Interesting, not recommended.
- **⑥ Cloud APIs (LANDR / eMastered / CloudBounce / RoEx Automaster / sonible).** Strong quality, mostly reference-free auto-master, RoEx/LANDR have APIs. **But cloud = against our local/private ethos** (upload every track, subscription). Skip unless the user wants a cloud option; eMastered = "reference-driven character", LANDR = consistent auto.

**Recommendation:** ship **① (curated gold-standard reference for Matchering)** now — smallest change, reuses Matchering, real-master quality, satisfies "no reference in mind." Add **② (pedalboard + pyloudnorm reference-free chain with LUFS target + genre tonal curve)** as the true no-reference engine + the loudness-target control users expect. Hold **③ phaselimiter** (box-side/Docker service) and **④ Ozone hosting** as higher-fidelity upgrades. Keep our reference-mode Matchering as-is for users who DO have a reference. (Sources below.)

**Sources (§16):** Matchering https://github.com/sergree/matchering · phaselimiter (MIT, reference-free) https://github.com/ai-mastering/phaselimiter + dataset (CC0) https://github.com/ai-mastering/bakuage_dataset1 · DeepAFx-ST https://github.com/adobe-research/DeepAFx-ST (paper https://arxiv.org/pdf/2207.08759) · pedalboard https://github.com/spotify/pedalboard · pyloudnorm https://github.com/csteinmetz1/pyloudnorm · 2026 AI-master comparison https://mixinggpt.com/blog/best-ai-mastering-plugins-2026 · LUFS/loudness targets https://beatstorapon.com/blog/spotify-loudness-2025-14-lufs-guide-ai-mastering-tips/ · iZotope Ozone 12 / Master Assistant https://www.oreateai.com/blog/exploring-the-best-ai-mastering-plugins-landr-vs-ozone-12/49924fd58d42111c50150f98998e9f68

## 17. Reference-to-Song analysis path ("analyze a track → build a Song spec → generate, NOT cover") — research (2026-05-27)

_User idea: upload a reference song → analyse bpm/key/structure/melody/(lyrics)/style → assemble a Song-Constructor arrangement pre-filled with that info → generate a SIMILAR song via normal **text2music** (avoiding the weak cover task). Asked for the best-possible tools per component, not just what we have._

**Verdict: HIGHLY FEASIBLE for everything except literal melody, and BETTER than what we run today.** It reuses the re-import-into-Song-Builder plumbing (commit f4e3990) — the analyser just emits a `song_meta` SongDraft that loads into the builder, the user tweaks, then generates.

**The one hard limit (be honest):** ACE-Step **text2music has no melody-conditioning input** — you cannot feed an extracted melody/MIDI into a from-scratch generation. So a generated "similar song" will match **bpm + key + structure + duration + style-tags + lyrics**, but NOT the literal tune. Extracted melody is only usable via the **Vocal Builder** (have it sing the transcribed melody) as an optional branch, or as on-screen reference. (Feeding melody as audio = the cover task, which we're deliberately avoiding.)

**Best-possible analysis stack (per component → tool → where it runs → vs what we have):**
- **Tempo + beats + downbeats + FUNCTIONAL STRUCTURE w/ labels → `allin1` (mir-aidj, All-In-One Music Structure Analyzer, ISMIR 2023).** One PyTorch model returns bpm, beats, downbeats, and **labelled** segments (intro/verse/chorus/bridge/outro) from demixed audio — exactly the Song-Constructor arrangement. **Big upgrade over our `sections.py`** (librosa agglomerative + energy-heuristic roles → real learned section labels) AND our `/api/beats` (librosa beat_track). Deps: torch + NATTEN + madmom + demucs; CPU/GPU/Apple-Silicon supported (NATTEN auto-installs on macOS) — but heavy → **deploy as a box-side GPU service (mirror RoFormer/SoulX); Mac-CPU is a slow fallback.** CLI `allin1 file.wav` / `allin1.analyze()`, JSON out @100fps.
- **Genre / mood / instrument auto-tags → CLAP (LAION) zero-shot** (preferred) or **Essentia MTG models.** CLAP embeds the audio and scores it against OUR genre/mood/instrument vocabulary (text prompts) → metal-subgenre-aware tags; PyTorch, cross-platform, runs on the box service. **Essentia** has a bigger pretrained zoo (Discogs-EffNet genre, MTG-Jamendo mood/instrument, key, tempo) but its models are **CC BY-NC-ND (non-commercial — fine for personal use)** and **`essentia-tensorflow` has broken osx-arm64 wheels (issue #1486) → would run box-side, not on the M4 Mac.** → Use **CLAP on the box** for tags; keep Essentia optional.
- **Key / scale → librosa chroma + Krumhansl–Schmuckler profiles** (pure-Python, Mac, ~30 lines) or **madmom `CNNKeyRecognition`** (already pulled in by allin1) or Essentia KeyExtractor (box). We have NO key detection today → any of these is new; the librosa/madmom route avoids Essentia-on-AS pain.
- **Lyrics → faster-whisper on the isolated vocal (Demucs/RoFormer) — ALREADY HAVE IT** (`/api/transcribe`). Singing ASR is imperfect but fine for pre-fill; user rates lyrics low-priority. (The ACE engine's `understand_music` is an alt.)
- **Melody → Basic Pitch (Spotify, MIT, polyphonic, light, Mac-CPU)** on the isolated vocal, or **CREPE** for a clean monophonic line. For the Vocal-Builder branch only (see hard limit). New dep, optional.

**Proposed deployment:** a **box-side `analyze` service** (3090, like RoFormer/SoulX): `allin1` (structure+bpm) + `CLAP` (tags) → JSON. **Mac-side**: whisper (lyrics, have it), key (librosa/madmom), Basic Pitch (melody, optional). New `/api/reference/analyze` orchestrates them → assembles a `song_meta` SongDraft (labelled blocks + per-section seconds, key, bpm, tags, optional lyrics) → loads into the Song Builder via the existing re-import path → user edits → text2music generate. No cover task touched.

**Phasing:** **P1 (mostly Mac, no GPU install): key (librosa) + structure/bpm via our existing `sections.py`+`/api/beats` + whisper lyrics → SongDraft → Song Builder.** Ships the whole workflow immediately on what we have. **P2: swap structure/bpm to the box `allin1` service** (proper labelled sections — the big quality jump) **+ CLAP tags.** **P3 (optional): Basic Pitch melody → Vocal Builder branch.** Confirm box install before P2 (no-surprise-GPU rule).

**Sources (§17):** allin1 https://github.com/mir-aidj/all-in-one (paper https://arxiv.org/abs/2307.16425) · Essentia models https://essentia.upf.edu/models.html (osx-arm64 wheel issue https://github.com/MTG/essentia/issues/1486) · CLAP https://github.com/LAION-AI/CLAP · Basic Pitch https://github.com/spotify/basic-pitch · CREPE https://github.com/marl/crepe · BeatNet https://github.com/mjhydri/BeatNet · Beat This! https://arxiv.org/pdf/2407.21658 · madmom https://madmom.readthedocs.io

### 17a. P2 BUILT + WORKING — the actual box install recipe (2026-05-27)

_P1 (Mac librosa) and P2 (box GPU) both shipped. P2 verified end-to-end: Mac `/api/reference/analyze` → `engine=allin1` with proper Intro/Verse/Chorus/Bridge labels + CLAP tags + key. Files: `backend/analyze_server.py` (box FastAPI :5075, `/health`+`/analyze`), `backend/analyze_py.py` (Mac client), `ANALYZE-API_AUTO_INSTALL.bat` (self-contained installer), `analyze_host` in app_config. The original allin1 path was a dead end; the working stack differs from the §17 plan:_

- **Use the `all-in-one-fix` fork (`import allin1fix`), NOT original `allin1`.** Original is pinned to natten 0.14.6 (old API `natten1dav`), whose prebuilt wheels are gone (shi-labs dead) → would need a CUDA-toolkit source build. The fork runs on modern torch + **natten 0.17.5** (which HAS prebuilt Windows wheels). The server/installer prefer `allin1fix`, fall back to `allin1`.
- **`all-in-one-fix` STILL needs madmom** — it `import madmom`s at runtime despite NOT declaring it in PyPI `requires_dist` (don't trust metadata; its own ImportError admits this). madmom compiles via git (`--no-build-isolation`) → needs the **MS C++ Build Tools** (the box has VS Build Tools 2022). Tags-only (CLAP) needs NO compiler; structure (allin1) does.
- **Pinned stack (no CUDA build):** Python **3.10–3.12** (natten wheels are cp310/311/312 only) · **torch 2.6.0 / cu126** (`torchvision==0.21.0 torchaudio==2.6.0`) — runs CLAP AND matches the natten wheel · prebuilt **`natten==0.17.5+torch260cu126-<cp>-...win_amd64.whl`** from `huggingface.co/lldacing/NATTEN-windows` · **madmom** from `git+https://github.com/CPJKU/madmom` · **all-in-one-fix** · **`numpy<2`** (laion-clap requires it; torch/all-in-one-fix re-bump it, so re-pin LAST).
- **Install order matters:** natten WHEEL before all-in-one-fix (else pip builds natten from sdist → fails); numpy<2 re-pinned at the very end.
- **Containment (user requirement):** PIP_CACHE_DIR pins only the download cache — pip's git-clone + wheel BUILD scratch uses **%TMP%/%TEMP%** (default `C:\Users\…\AppData\Local\Temp`). Must also set `TMP`/`TEMP` (+ HF/torch/XDG) inside the install dir, at install time AND in the launcher. The installer does this; delete-the-folder = clean uninstall.
...
- ~~Whether the installed engine build exposes `/v1/training/*`~~ — **VERIFIED 2026-05-27 on the box (`192.168.1.201:8001`, xl-sft + 4B LM loaded).** And it's bigger than just training: the engine exposes the **ENTIRE data→train→use pipeline over HTTP**, so Crucible can drive everything remotely with **no Gradio on the box.** Live routes: dataset `POST /v1/dataset/scan`, `/auto_label[_async]` (caption via the box LM) + `/auto_label_status`, `GET/PUT /v1/dataset/sample/{idx}`, `GET /v1/dataset/samples`, `/save`, `/preprocess[_async]` (encode→tensors, GPU) + `/preprocess_status`; training `POST /v1/training/start`, `/start_lokr`, `/stop`, `/export`, `/load_tensor_info`, `GET /v1/training/status`; inference `POST /v1/lora/load`, `/scale`, `/toggle`, `/unload`, `GET /v1/lora/status`. Status probes responded sane (`training:Idle`, `lora:unloaded`). **All live request schemas captured in [[METAL_LORA_PLAN.md]] §2.** The whole scan→label→save→preprocess→train→export→load→scale loop is reachable on our running engine — no rebuild needed. (Quirk: auto_label needs the LM loaded; preprocess/train need the DiT+decoder — swap via `/v1/init`↔`/v1/reinitialize`, the API equivalent of the tutorial's "restart Gradio to free VRAM.")

### 18a. LoRA vs LoKr — practical guidance (2026-05-27)
Same preprocessed tensors feed both (switching is cheap; the engine has `/v1/training/start` and `/start_lokr`). **LoKr** (Kronecker decomposition + DoRA on by default) trains **~10× faster** (≈5 min vs ≈1 h), smaller adapter files, comparable/often-better quality, lr 0.03 — but newer with some loader limitations. **LoRA** is the universal, battle-tested standard, lr 1e-4. **Recommendation: use LoKr for the experimentation phase** (fast cycles to dial in dataset/captions/epochs on the 3090), and a final **LoRA** only if you want max portability or LoKr quality disappoints. Both are "adapters" — small files that nudge the frozen base; neither retrains the 4B model.

**Full implementation plan + verified API contract: [[METAL_LORA_PLAN.md]].**
...
### 19b. Phase 0 DONE + in-app gate built (2026-06-13)
Box provisioned: 5 custom nodes (VideoHelperSuite, Frame-Interpolation, Yvann, GGUF, KJNodes) + 11 models (~64GB) all verified present; ComfyUI restarted, native Wan nodes confirmed live, GPU 24.5GB free. Stills model = Z-Image Turbo (ungated). User wanted the pipeline driven from the Crucible app, NOT the ComfyUI UI -> built a **Video tab** (App.tsx VIDEO group + forms.tsx VideoForm) with 3 gate actions over new backend routes /api/video/{still,i2v,lipsync} (backend/video.py builders + app.py on_complete_media + /api/media). Reuses the existing submit/WS/poll/library plumbing; user presses Generate in-app, ComfyUI stays headless. Verified: backend routes live (video flag true, validation OK), frontend builds clean (tsc+vite), Video tab renders with no console errors. NEXT = user runs the Phase 1 GO/NO-GO gate from the app (still -> animate -> lip-sync) and judges photoreal + metal-vocal lip-sync by eye. (The earlier CRUCIBLE_phase1_*.json ComfyUI templates are superseded by the app and can be ignored.)
