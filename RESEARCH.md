...
- ~~Whether the installed engine build exposes `/v1/training/*`~~ — **VERIFIED 2026-05-27 on the box (`192.168.1.201:8001`, xl-sft + 4B LM loaded).** `openapi.json` lists all of: `POST /v1/training/start`, `/start_lokr`, `/stop`, `/export`, `/load_tensor_info`, `GET /v1/training/status`, plus inference `POST /v1/lora/load`, `/scale`, `/toggle`, `/unload`, `GET /v1/lora/status`. `GET /v1/training/status` → `{is_training:false, status:"Idle", loss_history:[], …}`; `GET /v1/lora/status` → `{lora_loaded:false, adapters:[], scales:{}, …}`. The whole train→export→load→scale loop is reachable on our running engine — no rebuild needed.
