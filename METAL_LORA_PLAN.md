...
- **Phase 4 — Orchestration + UI** 🟡 _mostly built._ ✅ Backend `/api/lora/*` (4a): status preflight, dataset/add (Mac enrich+upload), scan, samples GET + sample PUT, autolabel, save, preprocess, train (LoRA/LoKr), status/stop, export(+load), inference load/scale/toggle/unload. ✅ Training tab (4b, `web/src/LoraTraining.tsx`, under **Lab**): explainer + live box-service preflight + 4 guided steps (add tracks w/ auto-label + lyrics-source badges; load+autocaption; train w/ params + live loss; export+load). ⬜ **TODO: the in-place review/EDIT table** — Step 2 currently scans + offers auto-caption but the per-track lyrics/caption editor (drive `samples` + `PUT sample/{idx}`) is still a stub note; the endpoints exist. ⬜ Box-dependent actions unrun (helper not installed + GPU). Serialize the GPU (free others first).
