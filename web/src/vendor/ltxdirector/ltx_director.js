/* ============================================================================================
   VENDORED from WhatDreamsCost-ComfyUI (GPL-3.0). Original: js/ltx_director.js.
   Two MusicGen-app patches only: (1) this shim header so the editor runs STANDALONE outside
   ComfyUI (no comfyAPI runtime); (2) an `export { TimelineEditor, parseInitial }` at the very end.
   The shim defines window.comfyAPI/app/LiteGraph if absent and routes api.* at the box ComfyUI
   via window.__LTXD_BOX__ (set by the React wrapper). Editor logic below is UNCHANGED.
   ============================================================================================ */
if (!window.comfyAPI) {
  const _base = () => (window.__LTXD_BOX__ || "");
  const _noop = () => {};
  const _app = { registerExtension: _noop, graph: { setDirtyCanvas: _noop }, canvas: null, canvasEl: null };
  const _api = { apiURL: (p) => _base() + p, fetchApi: (p, opts) => fetch(_base() + p, opts) };
  window.comfyAPI = { app: { app: _app }, api: { api: _api } };
  window.app = _app;
}
if (!window.LiteGraph) window.LiteGraph = { vueNodesMode: false };
...
/* MusicGen-app patch: expose the editor for standalone (React) mounting. */
export { TimelineEditor, parseInitial };
