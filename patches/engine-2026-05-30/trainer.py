...
class PreprocessedLoRAModule(nn.Module):
    ...
    def training_step(
        ...
        with autocast_ctx:
            ...
            # Sample timesteps. Crucible patch (2026-05-30): dispatch between
            # discrete turbo grid (current default) and continuous logit-normal
            # (matches the model's original sample_t_r training distribution —
            # see METAL_LORA_PLAN §12 + §13a.3). Continuous mode imports the
            # already-in-tree v2 implementation for single-source-of-truth.
            _ts_mode = getattr(self.training_config, "timestep_sampling_mode", "discrete")
            if _ts_mode == "continuous":
                from acestep.training_v2.timestep_sampling import sample_timesteps as _sample_cont
                _ts_mu = getattr(self.training_config, "timestep_mu", -0.4)
                _ts_sigma = getattr(self.training_config, "timestep_sigma", 1.0)
                t, _ = _sample_cont(
                    batch_size=bsz, device=self.device, dtype=self.dtype,
                    data_proportion=0.0, timestep_mu=_ts_mu,
                    timestep_sigma=_ts_sigma, use_meanflow=False,
                )
            else:
...
class PreprocessedLoKRModule(nn.Module):
    ...
    def training_step(
        ...
        Crucible patch (2026-05-29): accept ``record_loss`` like the LoRA module
        so the LoKr fabric trainer can run validation passes without polluting
        ``training_losses``.
        """
        if self.device_type in ("cuda", "xpu", "mps"):
            autocast_ctx = torch.autocast(
        ...
        with autocast_ctx:
            ...
            # Crucible patch (2026-05-30): see twin block in PreprocessedLoRAModule
            # for rationale. Default discrete preserves all prior behavior.
            _ts_mode = getattr(self.training_config, "timestep_sampling_mode", "discrete")
            if _ts_mode == "continuous":
                from acestep.training_v2.timestep_sampling import sample_timesteps as _sample_cont
                _ts_mu = getattr(self.training_config, "timestep_mu", -0.4)
...
class LoKRTrainer:
    ...
    def _train_with_fabric(
        ...
        train_loader = data_module.train_dataloader()
        # Crucible patch (2026-05-29): wire val_loader through so val_split>0
        # actually drives validation + best-checkpoint tracking. The data_module
        # already honors val_split, but the upstream LoKr trainer never pulled
        # the val_loader out — meaning best-checkpoint saves were dead code.
        val_loader = (
            data_module.val_dataloader()
            if hasattr(data_module, "val_dataloader")
            else None
        ...
        accumulated_loss = 0.0
        global_step = 0
        # Crucible patch (2026-05-29): best-checkpoint tracking state, mirrors
        # the LoRA fabric trainer at L778. Stays inert when val_loader is None.
        best_val_loss = float("inf")
        best_val_step = None
        if training_state is not None:
        ...
        for epoch in range(self.training_config.max_epochs):
            ...
            # Crucible patch (2026-05-29): per-epoch validation + best-checkpoint
            # save. Only runs when a val_loader exists (val_split>0). Mirrors the
            # LoRA fabric trainer's block at L1097-1127 but saves LoKr-format
            # weights via save_lokr_training_checkpoint.
            if val_loader is not None:
                self.module.model.decoder.eval()
                total_val_loss = 0.0
                n_val = 0
                with torch.no_grad():
                    for val_batch in val_loader:
                        v_loss = self.module.training_step(
                            val_batch, record_loss=False
                        )
                        v = v_loss.item()
                        if v == v:  # NaN guard
                            total_val_loss += v
                ...
                if n_val > 0:
                    ...
                    self.fabric.log("val/loss", val_loss, step=global_step)
                    if training_state is not None:
                        training_state["plot_val_steps"].append(global_step)
                        training_state["plot_val_loss"].append(val_loss)
                    yield (
                        global_step,
                        val_loss,
                        f"🧪 Val loss @ epoch {epoch + 1}: {val_loss:.4f}",
                    )
                    if val_loss < best_val_loss:
                        best_val_loss = val_loss
                        best_val_step = global_step
                        ...
                        yield (
                            ...
                            val_loss,
                            (
                                f"🏆 New best val_loss {val_loss:.4f} at epoch "
                                f"{epoch + 1} → checkpoints/best/"
                            ),
                        )
